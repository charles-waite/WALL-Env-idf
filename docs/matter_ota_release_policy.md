...
3. Choose the next monotonic version.
4. Build with `tools/build-ota.sh <xiao|supermini> --version X.Y.Z`.
5. Inspect the OTA header and generated JSON.
6. Copy both generated files to the Matter Server OTA directory.
7. Restart the Matter Server add-on.
8. Use the Matter Server UI or HA update entity to verify the update source is
   `local`.
9. Update one test node first.
10. Confirm download, reboot, rollback validation, and new version reporting.
11. Leave the OTA files in place only while that version should remain
    available.
